# 作业提交系统（Redrock Assessment System）

## 项目简介

本项目是一个面向内部使用的作业提交系统，支持管理员/讲师发布与批改作业、学员提交作业并查看评语的完整流程。系统按学科划分权限，涵盖后端、前端、SRE、产品、视觉设计、Android、iOS七个方向，并集成了 AI 初评功能以辅助教学评估。
...
- **用户模块**
  ...
  - 获取当前用户信息（含 `subject` 与 `subject_label`）
  - 注销账号（软删除）
  - 密码使用 bcrypt 加盐哈希存储
  - JWT 双 Token 认证机制（含刷新逻辑）

- **作业模块**
  - 老师可发布作业（设置标题、描述、学科、截止时间、是否允许补交）
  - 按学科筛选 + 分页查询作业列表
  - 查看作业详情（含发布者信息、提交人数、当前用户提交状态）
  - 同学科老师可修改或删除作业（已处理并发控制）

- **提交模块**
  - 学生提交作业（自动判断是否迟交）
  - 学生查看自己的所有提交及评语
  - 老师查看本学科所有学员的提交
  - 老师批改作业（填写分数、评语、标记是否优秀）
  - 所有用户可查看优秀作业列表（支持按学科筛选）

- **其他要求**
  - 统一响应格式：`{ code, message, data }`
  - 学科枚举值与中文标签同时返回（如 `"backend"` → `"后端"`）
...
- **AI 作业初评（AI Review）**
  - 老师可通过 `/submission/:id/aiReview` 接口触发 AI 对作业内容的自动分析
  - 调用大模型 API对代码质量、结构、规范性进行初步评价
  - 返回 AI 生成的评语建议与推荐分数，供老师参考
  - 支持文本/链接形式的作业内容分析
